<template>
    <div style="background-color:rgba(73, 219, 255, 0.3)">
        <div style="background-color:rgba(63, 157, 181, 0.3)">
            <el-descriptions title="店铺信息">
                ...
                <el-descriptions-item label="状态">
                    ...
                    <el-tag size="small" v-if="business.status == 0">封禁</el-tag>
        ...
        &nbsp;
        <router-link to="/businesshome/businessorder"><el-button type="primary" plain><el-icon>
                    <Tickets />
                </el-icon>订单管理</el-button></router-link>
        &nbsp;
        <router-link to="/businesshome/businessgoods"><el-button type="primary" plain> <el-icon>
                    <Goods />
                </el-icon>
                商品管理</el-button></router-link>
        <el-dialog v-model="outerVisible" title="更改店铺信息" append-to-body>
            <template #default>
                <table>
                    <tr>
                        <td>商户名称</td>
                        <td><el-input v-model="business.name" type="text" placeholder="请输入店铺名" style="width:300px" /></td>
                    </tr>
                    <tr>
                        <td>商户账号</td>
                        <td><el-input v-model="business.username" type="text" disabled placeholder="请输入店铺名"
                                style="width:300px" /></td>
                    </tr>
                    <tr>
                        <td>商户头像</td>
                        <td><el-upload class="avatar-uploader" action="/api/photo/business"
                                :on-success="handleAvatarSuccess">
                                <img v-if="business.img" :src="business.img" class="avatar" />
...
const getBusiness = () => {
    axios.get('/api/business/' + id).then((res) => {
        if (res.data.code == 1) {
            ...
            password.value = business.value.password
            selectedOptions.value=business.value.address.split('/')
        }
    })
}
getBusiness()
...
const handleChange = () => {
    ...
    business.value.address = province.value +"/"+ city.value +"/"+ country.value
}
...
const mima = () => {
    ...
    } else if (md5(pass.value) == business.value.password) {
        istrue.value = true
...
const update = () => {
    if (istrue.value == false) {business.value.password=undefined}
    console.log(business.value)
    axios.put('/api/business', business.value).then((res) => {
        if (res.data.code == 1) {
