  ...
  <div class="common-layout">
    <el-container>
      <el-aside width="200px">
        <el-input
          v-model="input"
          placeholder="请输入查询的用户名/id"
          input-style="width:150px;"
          @keyup.enter.native="getuserbyid(input)"
        ></el-input>
        <el-scrollbar>
          <el-menu v-infinite-scroll="load" style="overflow: auto;height:450px">
            <el-menu-item
              ...
              :key="user.id"
              @click="gettalkuser(user.id)"
              divided
            >
              <el-badge v-if="user.num > 0" :value="user.num" class="item">
      ...
      <el-container>
        <el-header height="50px">
          <div style="text-align: center;">
            <span style="font-size: 2em;">{{ talkuser.name }}</span>
          </div>
        </el-header>
        <el-main>
          <div class="talkbox">
            ...
            <div
              v-for="msg in msgList.msgs"
              :key="msg.id"
              v-infinite-scroll="load"
              style="overflow: auto"
            >
              <div class="time">
                <span style="font-size: small;">{{ msg.createTime }}</span>
              </div>
              <div class="talk">
                <div class="leftuser" v-if="msg.type == 'receive'">
                  <img :src="talkuser.img" class="avatar" />
                  <span>{{ talkuser.name }}</span>
                </div>
                <div class="rightuser" v-if="msg.type == 'sender'">
                  <img :src="self.img" class="avatar" />
                  {{ self.name }}
                </div>
        ...
        <el-footer>
          <el-input
            ...
            type="textarea"
            placeholder="请输入内容"
            @keyup.enter.native="sender(msg111)"
          />
          <br />
          <div style="float: right;">
            <el-button plain @click="sender(msg111)">发送消息</el-button>
          </div>
        </el-footer>
      </el-container>
    </el-container>
...
const selfid = sessionStorage.getItem("userid");
...
const getself = () => {
  axios.get("/api/user/" + selfid).then(res => {
    if (res.data.code == 1) {
...
const getMessage = talkid => {
  msgList.msgs = [];
  axios
    .put("/api/message", { sender: talkid, receive: selfid, isnew: 0 })
    .then(res => {
      getUser();
      axios
        .post("/api/message/message", { sender: talkid, receive: selfid })
        .then(res => {
          if (res.data.code == 1) {
            for (const item of res.data.data) {
              const message = item;
            ...
            axios
              .post("/api/message/message", { sender: selfid, receive: talkid })
              .then(res => {
                if (res.data.code == 1) {
                  for (const item of res.data.data) {
                    const message = item;
                  ...
                  for (let i = 0; i < msgList.msgs.length - 1; i++) {
                    for (let j = i; j < msgList.msgs.length; j++) {
                      if (
                        msgList.msgs[i].createTime > msgList.msgs[j].createTime
                      ) {
